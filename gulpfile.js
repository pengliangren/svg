...
gulp.task('upload_zip', ['compress'], () => {
  return gulp.src('./publish/publish.zip')
  .pipe(upload({
    url: "http://wapstatic.sparta.html5.qq.com/upload",
    data: {
      type: 'zip',
      to: `/data/wapstatic/${config.upload.userName}/${config.upload.projectName}`
    },
    callback: () => {
      console.info(colors.green(`文件已经上传到 http://wapstatic.sparta.html5.qq.com/${config.upload.userName}/${config.upload.projectName} 目录下`))
      del.sync(['./publish/publish.zip'])
    },
    timeout: 60000
  }))
...
gulp.task('del_zip', ['upload_zip'], () => {
  del.sync(['./publish/publish.zip'])
})

gulp.task('upload', ['del_zip'])
